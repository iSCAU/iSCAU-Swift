...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // Swift
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showNotice:) name:@"kShowNoticeNotification" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hideNotice:) name:@"kHideNoticeNotification" object:nil];
}
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    // Swift
    [[NSNotificationCenter defaultCenter] removeObserver:@"kShowNoticeNotification"];
    [[NSNotificationCenter defaultCenter] removeObserver:@"kHideNoticeNotification"];
}
...
- (void)onlineConfigCallBack:(NSNotification *)notification {
    ...
    if (note && (![note isEqualToString:@"0"])) {
        NSString *notice = [[NSUserDefaults standardUserDefaults] objectForKey:NOTIFICATION];
        if (![note isEqualToString:notice]) {
