...
- (void)calculateDaysInCurrentMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    for (NSInteger i = 1; i < daysCount + 1; ++i) {
        CalendarDayModel *calendarDay = [CalendarDayModel calendarDayWithYear:components.year month:components.month day:i];
...
- (void)changStyle:(CalendarDayModel *)calendarDay
{
    ...
    if (calendarToDay.year == calendarDay.year &&
        ...
        if (calendarDay.day == 1) {
            calendarDay.holiday = [NSString stringWithFormat:@""SINT"\n国庆节", calendarDay.day];
        }
        calendarDay.style = CellDayTypeWeek;
        ...
        // 补课
        calendarDay.holiday = [NSString stringWithFormat:@""SINT"\n补课", calendarDay.day];
        calendarDay.style = CellDayTypeNotice;
        
    } else if (calendarDay.month == 11 &&
               (calendarDay.day == 28 || calendarDay.day == 29)) {
        calendarDay.holiday = [NSString stringWithFormat:@""SINT"\n校运会", calendarDay.day];
        calendarDay.style = CellDayTypeWeek;
    } else if (calendarDay.month == 1 &&
               calendarDay.day >= 5 && calendarDay.day <= 11) {
        calendarDay.holiday = @"新生考试周";
        calendarDay.style = CellDayTypeNotice;
    } else if (calendarDay.month == 1 &&
               calendarDay.day >= 12 && calendarDay.day <= 25) {
        calendarDay.holiday = @"考试\n军训";
        calendarDay.style = CellDayTypeNotice;
    } else if ((calendarDay.month == 1 && calendarDay.day >= 26) ||
               calendarDay.month == 2 ||
               (calendarDay.month == 3 && calendarDay.day <= 8)) {
                calendarDay.holiday = [NSString stringWithFormat:@""SINT"\n寒假", calendarDay.day];
        calendarDay.style = CellDayTypeWeek;
    }
...
-(NSString *)LunarForSolarYear:(NSInteger)wCurYear Month:(NSInteger)wCurMonth Day:(NSInteger)wCurDay{
    ...
    //公历每月前面的天数
    const NSInteger wMonthAdd[12] = {0,31,59,90,120,151,181,212,243,273,304,334};
    
    //农历数据
    const NSInteger wNongliData[100] = {2635,333387,1701,1748,267701,694,2391,133423,1175,396438
